    ...
    public APIResponse authenticate() {
        ...
        if (completionDate != null && !completionDate.isEmpty()) {
            try {
                ...
                try {
                    date = LocalDate.parse(completionDate, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                } catch (DateTimeParseException ex1) {
                    // fallback -> dd/MM/yyyy
                    date = LocalDate.parse(completionDate, DateTimeFormatter.ofPattern("dd/MM/yyyy"));
                }
                ...
                return new APIResponse(
                        "BAD_REQUEST",
                        HttpStatus.BAD_REQUEST.value(),
                        "Invalid completion date format (expected yyyy-MM-dd or dd/MM/yyyy)",
                        null
                );
            }
        }
